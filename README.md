...
<li>使用 $emit 和 $on 解决了跨组件的数据传递</li>
<li>在面包屑中添加 onclick 失效，可以在 onclick 后加入 native 监听原生事件</li>
<li>为每个组件添加 url 数据，每次创建时都将导航栏修改为实际访问页面</li>
<li>目前导航栏的所有 bug 已经修复</li>
<li>修复了详细拓展列的 bug，span 应该使用动态绑定</li>
<li>修复了日期选择器和性别选择器的错误，由于数据库中读取到的数据都是通过转换之后才绑定在选项中的，所以如果用户没有修改性别，性别就仍是转换之后的绑定值，如果用户修改了性别，就会绑定性别的 value 值，然后二次转换导致错误。解决方案是将 label 和 value 统一</li>
<li>修复了删除数据后如果当前页没有任何数据，此页仍存在的 bug，现在删除数据并分页时会判断是否为空，如果删光了本页的数据就会显示上一页的数据</li>
<li>考虑到服务器性能比较低，为大部分操作添加了 loading 操作</li>
<li>发现一个分页的 bug，搜索之后取消再次获取列表数据， currentPage 的绑定会失效，必须再次点击，在绑定后加 .sync 可以解决</li>

### 2020.8.20 - Demo 0.9.0
<li>更新了 API 文档管理</li>

### 2020.8.21 - Demo 0.9.1
<li>将背景图和首页大图放到了项目里，因为使用外链加载会很慢</li>
